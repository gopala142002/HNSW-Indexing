...
class HierarchicalNSW : public AlgorithmInterface<dist_t> 
{
    ...
    priority_queue<pair<dist_t, tableint>, vector<pair<dist_t, tableint>>, CompareByFirst>
...
//                    if (candidate_id == 0) continue;
    ...
    vector<data_t> getDataByLabel(labeltype label) const {
    ...
    */
    ...
    void repairConnectionsForUpdate(
        ...
        for (int level = dataPointLevel; level >= 0; level--) {
            priority_queue<pair<dist_t, tableint>, vector<pair<dist_t, tableint>>, CompareByFirst> topCandidates = searchBaseLayer(
    ...
    // walk of HNSW (levels maxlevel_ -> 1) and instead uses VTree greedy search
    // to obtain a high-quality entry point directly into level 0.
    //
    // Algorithm:
    //   1. Call vtree_->searchNN(query) to get the best candidate from
    //      the Voronoi tree (greedy traversal).
    //   2. Use this candidate as the entry point for level 0 search.
    //      Falls back to enterpoint_node_ if vtree_ is null or returns no result.
    //   3. Run searchBaseLayerST from currObj on level 0 -- identical to what
    //      searchKnn() does after its upper-layer walk.
    //
    // Parameters:
    //   query_data  -- raw query vector (same pointer convention as searchKnn)
    //   k           -- number of nearest neighbours to return
    //   isIdAllowed -- optional filter functor (same as searchKnn)
    ...
    searchKnnVTree(
        ...
        // ── Step 1: VTree greedy search → best candidate
        tableint currObj = enterpoint_node_;   // safe default
        dist_t   curdist = fstdistfunc_(query_data,
                                         getDataByInternalId(enterpoint_node_),
                                         dist_func_param_);

        if (vtree_ != nullptr) {
            // searchNN expects a const float* query
            const float* query_f = reinterpret_cast<const float*>(query_data);
            vector<int> seeds = vtree_->searchNN(query_f);

            // ── Step 2: pick the seed closest to the query
            for (int seed_id : seeds) {
                if (seed_id < 0 || static_cast<size_t>(seed_id) >= cur_element_count)
                    continue;   // guard against stale / out-of-range IDs
        ...
        // If vtree_ is null we simply fall through with the global enterpoint.

        // Step 3: level-0 beam search from the chosen entry point 
        // Identical to the tail of searchKnn(); skips all upper-layer hops.
    ...
    //
    // Drop-in replacement for searchKnn() that bypasses the upper-layer greedy
    // walk of HNSW and instead uses the M-tree to obtain a single
    // high-quality entry point directly into level 0.
    ...
    searchKnnMTree(
        ...
        if (mtree_ != nullptr) {
            const float* query_f = reinterpret_cast<const float*>(query_data);
            const int seed = mtree_->greedySearch(query_f);
    ...
    //
    // Drop-in replacement for searchKnn() that bypasses the upper-layer greedy
    // walk of HNSW (levels maxlevel_ -> 1) and instead uses PCTree greedy search
    // to obtain a high-quality entry point directly into level 0.
    //
    // Algorithm:
    //   1. Call pctree_->searchNN(query) to get the best candidate from
    //      the PCA tree (greedy traversal).
    //   2. Use this candidate as the entry point for level 0 search.
    //      Falls back to enterpoint_node_ if pctree_ is null or returns no result.
    //   3. Run searchBaseLayerST from currObj on level 0 -- identical to what
    //      searchKnn() does after its upper-layer walk.
    //
    // Parameters:
    ...
    searchKnnPCTree(
        ...
        // ── Step 1: PCTree greedy search → best candidate
        tableint currObj = enterpoint_node_;   // safe default
        dist_t   curdist = fstdistfunc_(query_data,
                                         getDataByInternalId(enterpoint_node_),
                                         dist_func_param_);
        ...
        // If pctree_ is null we simply fall through with the global enterpoint.

        // Step 3: level-0 beam search from the chosen entry poin
        // Identical to the tail of searchKnn(); skips all upper-layer hops.
        priority_queue<pair<dist_t, tableint>,
                            vector<pair<dist_t, tableint>>,
                            CompareByFirst> top_candidates;
