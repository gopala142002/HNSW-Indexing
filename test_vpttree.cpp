...
int main(int argc, char** argv) {
    ...
    std::cout << "\nBuilding VantagePointTree (leafCapacity=" << leaf_cap << ")...\n";
    ...
    std::cout << "VantagePointTree built in " << std::setprecision(4) << total_time << " s.\n";

    std::cout << "VantagePointTree height: " << hnsw.getVPTreeHeight() << "\n\n";

    std::cout << "Running " << Q << " queries (k=" << k << ", ef=" << ef_search << ")\n\n";

    double total_hnsw_recall = 0.0;
    double total_vpt_recall = 0.0;
    long total_query_time_hnsw = 0;
    long total_query_time_vpt = 0;
    ...
    for (int q = 0; q < Q; ++q) 
    {
        ...
        total_query_time_vpt += std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count();

        std::vector<size_t> vpt_ids;
        vpt_ids.reserve(k);
        while (!pq_vpt.empty()) {
            vpt_ids.push_back(pq_vpt.top().second);
            pq_vpt.pop();
        }

        total_hnsw_recall  += recall_at_k(gt_q, gt_dim, hnsw_ids,  k);
        total_vpt_recall += recall_at_k(gt_q, gt_dim, vpt_ids, k);
    }

    float avg_recall_hnsw  = static_cast<float>(total_hnsw_recall)  / Q;
    float avg_recall_vpt = static_cast<float>(total_vpt_recall) / Q;
    float avg_time_hnsw_us  = static_cast<float>(total_query_time_hnsw)  / Q;
    float avg_time_vpt_us = static_cast<float>(total_query_time_vpt) / Q;

    std::cout << std::fixed << std::setprecision(4);
    
    std::cout << "GIST Results (N=" << N
          << ", dim=" << dim
          << ", k=" << k
          << ", ef=" << ef_search << ")\n\n";
    ...
    std::cout << "VPTree -> level-0:   Recall@" << k << " = " << std::setprecision(4) << avg_recall_vpt
        << ", Avg latency (us) = " << std::fixed << std::setprecision(1) << avg_time_vpt_us << "\n";

    std::cout << "\n";
    std::cout << "Speedup (HNSW / VPTree):     " << std::fixed << std::setprecision(2)
            << (avg_time_hnsw_us / avg_time_vpt_us) << "x\n";
    std::cout << "Recall loss (HNSW - VPTree): " << std::fixed << std::setprecision(4)
            << (avg_recall_hnsw - avg_recall_vpt) << "\n";
